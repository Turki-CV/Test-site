  ...
  document.querySelectorAll(".tilt-card").forEach(card => {
    card.addEventListener("pointermove", event => {
      ...
      card.style.transform =
        `perspective(1100px) rotateX(${(-y * 2.8).toFixed(2)}deg) ` +
        `rotateY(${(x * 3.2).toFixed(2)}deg) translateY(-5px)`;
    });
...
form?.addEventListener("submit", async event => {
  ...
  try {
    ...
    if (status) {
      status.textContent =
        "تعذر إرسال الرسالة حاليًا. يمكنك التواصل عبر البريد الإلكتروني مباشرة.";
...
(() => {
  ...
  topButtons.forEach(el => {
    el.addEventListener("click", goToTop, { passive: false });
  });
